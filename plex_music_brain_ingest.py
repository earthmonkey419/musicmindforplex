    ...
    for i, artist in enumerate(artists, 1):
        for album in artist.albums():
            for track in album.tracks():
                ...
                # Incremental skip — process tracks added OR updated since last ingest
                if last_ingest:
                    added_new = track.addedAt and track.addedAt.isoformat() > last_ingest
                    updated_new = track.updatedAt and track.updatedAt.isoformat() > last_ingest
                    if not added_new and not updated_new:
                        skipped += 1
                        continue
